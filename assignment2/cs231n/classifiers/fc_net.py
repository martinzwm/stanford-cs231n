...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        _, dW1, db1 = affine_relu_backward(dH, H_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            self.params['W'+str(i)] = weight_scale*np.random.randn(hidden_dims[i], hidden_dims[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            X, cache['layer'+str(i)] = affine_norm_relu_do_forward(X, W, b, 
                     gamma, beta, bn_params, self.normalization, 
                     self.use_dropout, self.dropout_param)
        
        # Last layer, only fc. No normalization or activation
        W = self.params['W'+str(self.num_layers-1)]
        b = self.params['b'+str(self.num_layers-1)]
        scores, cache['layer'+str(self.num_layers-1)] = affine_forward(X,W,b)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dH, dW, db = affine_backward(dscores, cache['layer'+str(self.num_layers-1)])
        grads['W'+str(self.num_layers-1)] = dW + self.reg*self.params['W'+str(self.num_layers-1)]
        grads['b'+str(self.num_layers-1)] = db
        
        # Gradients of the 1st to the second last layer
        for i in range(self.num_layers-2, -1, -1):
            dH, dW, db, dgamma, dbeta = affine_norm_relu_do_backward(dH, 
                                                                cache['layer'+str(i)])
