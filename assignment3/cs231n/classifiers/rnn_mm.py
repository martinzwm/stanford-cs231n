...
class CaptioningRNN(object):
    ...
    # During training only. Use self.sample() during testing.
    def loss(self, features, captions):
        ...
        ### FORWARD PASS ###
        # (1) use features to compute initial hidden state
        h0 = np.dot(features, W_proj) + b_proj
        
        # (2) convert captions into vectors using dictionary W_embed
        x, x_cache = word_embedding_forward(captions_in, W_embed)
        
        # (3) compute the hidden states using rnn or lstm
        if self.cell_type == 'rnn':
            H, H_cache = rnn_forward(x, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            H, H_cache = lstm_forward(x, h0, Wx, Wh, b)
            
        # (4) compute scores from the hidden states 
